	...
	<%
		...
		try {
			...
			con = DriverManager.getConnection(
					"jdbc:mysql://localhost:3306/lms", "root", "password");
		...
		if (flag1 == true) {
			try {
				...
				con = DriverManager.getConnection(
						...
						"password");
				stmt = con.createStatement();
				rs = stmt.executeQuery("select id from issue");
				while (rs.next()) {
					if (rs.getInt(1) == id) {
						flag2 = true;
					}
				...
				if (flag2 == true) {
					...
					String executeQuery = "insert into issue values("
							+ isbn + "," + id + ",'" + date + "')";
		...
		if (flag == true) {
			try {
				...
				con = DriverManager.getConnection(
						...
						"password");
				stmt = con.createStatement();
				rs2 = stmt.executeQuery("select availablebooks from book_availability where isbn="+ isbn + "");
				rs2.next();
				int i = rs2.getInt(1);
				i--;
				String executeQuery1 = "update book_availability set availablebooks =" + i
						+ " where isbn = " + isbn + "";
				stmt.executeUpdate(executeQuery1);
